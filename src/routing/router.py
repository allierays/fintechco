...
def score_rail(rail: dict, amount: float) -> float:
    ...
    The cost_weight is dynamically adjusted based on the transfer amount:
    - For large transfers ($10k+), success_rate dominates (weight 0.8)
    - For small transfers (<$500), cost dominates (weight 0.8)
    - For mid-range transfers, a balanced 0.5/0.5 split is used
    """
    if amount >= 10_000:
        cost_weight = 0.2
        success_weight = 0.8
