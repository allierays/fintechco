...
def score_rail(rail: dict, amount: float) -> float:
    ...
    The weight given to cost vs. success rate is dynamically adjusted
    based on the transfer amount. For large transfers ($10k+), success
    rate dominates (weight 0.8). For small transfers (<$500), cost
    dominates (weight 0.8).
    """
    if amount >= 10_000:
        cost_weight = 0.2
        success_weight = 0.8
