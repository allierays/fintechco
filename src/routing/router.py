...
def score_rail(rail: dict, amount: float) -> float:
    """
    Score a payment rail based on its success rate and cost.

    The scoring function is dynamic and adjusts the weight of success rate vs. cost
    based on the transfer amount:
    - For large transfers ($10k+), success rate dominates with a weight of 0.8.
    - For small transfers (<$500), cost dominates with a weight of 0.8.
    - For medium-sized transfers, the weights are balanced at 0.5/0.5.
    """
    if amount >= 10000:
        cost_weight = 0.2
        success_weight = 0.8
