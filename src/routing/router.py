...
def score_rail(rail: dict, amount: float) -> float:
    ...
    The scoring function dynamically adjusts the weights for cost and success rate
    based on the transfer amount:
    - For large transfers ($10k+), success_rate is weighted more heavily (0.8)
    - For small transfers (<$500), cost is weighted more heavily (0.8)
    - For mid-range transfers, the weights are balanced (0.5/0.5)
    """
    if amount >= 10_000:
        cost_weight = 0.2
        success_weight = 0.8
